    ...
    <div class="row g-3">
        <div class="col-lg-5"><div class="card border-0 shadow-sm"><div class="card-body">
            <form wire:submit.prevent="save">
                ...
                <button class="btn btn-success">Record Drug Chart</button>
            </form>
        </div></div></div>
        <div class="col-lg-7"><div class="card border-0 shadow-sm"><div class="table-responsive">
            <table class="table table-hover mb-0"><thead class="table-light"><tr><th>Time</th><th>Medicine</th><th>Dosage</th><th>Comment</th></tr></thead><tbody>
                @forelse($recent as $chart)
                    <tr><td>{{ $chart->time }}</td><td>{{ $chart->medicine?->name }}</td><td>{{ $chart->dosage }}</td><td>{{ $chart->comment }}</td></tr>
                @empty
                    <tr><td colspan="4" class="text-center text-muted py-4">No drug chart entries yet.</td></tr>
                @endforelse
            </tbody></table>
        </div></div></div>
    </div>
