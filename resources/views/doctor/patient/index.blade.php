...
<div class="container-fluid">
    <div class="row mb-4">
        <div class="col-md-8">
            ...
            <p class="text-muted">Manage patients under doctor's care</p>
        </div>
    </div>  
    @if(count($requests)== 0)
        <div class="alert alert-info">No Doctor Patients records found.</div>
    @else
        <div class="card">
                        ...
                        <tbody>
                            @foreach($requests as $request)
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <a href="{{ route('doctor.patient.show', $request->patientVisit->patient) }}" class="btn btn-sm btn-info"><i class="bi bi-eye"></i> View Patient Profile</a>
                                        @if($request->status != 'completed')
                                        <a href="{{ route('doctor.patient.complete', $request) }}" onclick="return confirm('Are you sure you want to mark this request as completed?');" class="btn btn-sm btn-success"><i class="bi bi-check-circle"></i> Mark as Completed</a>
                                        @endif
                                        <!-- close visit -->
                                        <a href="{{ route('doctor.patient.close-visit', $request->patientVisit) }}" onclick="return confirm('Are you sure you want to close this visit?');" class="btn btn-sm btn-warning"><i class="bi bi-x-circle"></i> Close Visit</a>
                                    </td>
                                </tr>
