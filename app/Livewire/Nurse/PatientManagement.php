...
class PatientManagement extends Component
{
    ...
    {
        // The poll re-renders the component so new service requests appear automatically.
    }
    ...
    public function completeRequest(int $requestId): void
    {
        ...
        $serviceRequest->patientVisit?->visitActivities()->create([
            'recorded_by' => auth()->id(),
            'activity' => 'Service "' . ($serviceRequest->service?->name ?? 'Nursing service') . '" marked as completed by ' . auth()->user()->name,
        ]);
